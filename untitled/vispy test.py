...
# encoding: utf8
"""
Vispy 3d scatter plot with clickable markers demo.
...
def scatter3d(positions, colors, symbol='o', size=20, click_radius=2,
              on_click=None):
    """
    ...
    if on_click is not None:
        def on_mouse_release(event):
            if event.trail() is not None and event.button == 1 and distance_traveled(event.trail()) <= 2:
...
if __name__ == "__main__":
    ...
    def on_click(idx, markers):
        ...
        colors[idx] = (1, 1, 1, 1)
        markers.set_data(data, face_color=colors, symbol='o', size=20,
                         edge_color=None)
